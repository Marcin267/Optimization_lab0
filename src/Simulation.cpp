...
Simulation::Simulation()
{
    ...
    random_count = genetic_config.population_size * genetic_config.generations;
...
void Simulation::run(nlohmann::json config) const
{
    ...
    {
        std::string results_path = "results/" + problem_file.get<std::string>() + "/";
        std::filesystem::create_directories(results_path);

        std::string instance_file_name = "instances/" + problem_file.get<std::string>();
        Problem problem(instance_file_name);

        for (int i = 0; i < genetic_config.count; i++)
            GeneticAlgorithm genetic_algorithm(problem, genetic_config.population_size ,
                genetic_config.generations,genetic_config.cross_probability, genetic_config.mutation_probability,
